...
class Unpack extends Parser {
  ...
  [STRIPABSOLUTEPATH] (entry, field) {
    ...
    if (!raw || this.preservePaths) return true

    const { type } = entry
    const p = normPath(String(raw))
    // strip off the root (CVE-2026-29786 / GHSA-qffp-2rhf-9h96)
    const [root, stripped] = stripAbsolutePath(p)
    const parts = stripped.replace(/\\/g, '/').split('/')

    /* istanbul ignore next */
    const winDriveDotDotSegment =
      isWindows && /^[a-z]:\.\.$/i.test(parts[0] || '')
    if (parts.includes('..') || winDriveDotDotSegment) {
      if (field === 'path' || type === 'Link') {
        this.warn(`${field} contains '..'`, p)
        return false
      }
      const entryDir = path.posix.dirname(String(entry.path).split(/\\/g).join('/'))
    ...
    if (root) {
      if (field !== 'path' && type !== 'Link' && parts.includes('..')) {
  ...
  [CHECKPATH] (entry) {
    ...
    if (this.strip) {
      ...
      p = normPath(entry.path)
      parts = p.split('/')
    }

    if (isFinite(this.maxDepth) && parts.length > this.maxDepth) {
      this.warn('path excessively deep', {
  ...
  [SYMLINK] (entry, done) {
    ...
    this[ENSURE_NO_SYMLINK](
      ...
      this.cwd,
      parts,
      function () { self[LINK](entry, entry.linkpath, 'symlink', done) },
      function (er) { self[ONERROR](er, entry) }
    )
  }
  ...
  [HARDLINK] (entry, done) {
    ...
    this[ENSURE_NO_SYMLINK](
      ...
      this.cwd,
      parts,
      function () { self[LINK](entry, linkpath, 'link', done) },
      function (er) { self[ONERROR](er, entry) }
    )
  }
  ...
  [ENSURE_NO_SYMLINK] (entry, cwd, parts, done, onError) {
    ...
    fs.lstat(t, function (er, st) {
      ...
      self[ENSURE_NO_SYMLINK](entry, t, parts, done, onError)
    })
  }
